(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))u(a);new MutationObserver(a=>{for(const t of a)if(t.type==="childList")for(const r of t.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&u(r)}).observe(document,{childList:!0,subtree:!0});function n(a){const t={};return a.integrity&&(t.integrity=a.integrity),a.referrerPolicy&&(t.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?t.credentials="include":a.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function u(a){if(a.ep)return;a.ep=!0;const t=n(a);fetch(a.href,t)}})();const A="/assets/logo-DGPpP51Y.png";function N(e){e.innerHTML=`
    <nav class="navbar">
  ...
  `;const o=e.querySelector(".navbar-toggle"),n=e.querySelector(".navbar-menu");if(o&&n){const u=Array.from(n.querySelectorAll("li[data-route]")),a=()=>{const s=window.location.pathname||"",i=s.split("/").pop()||"",v=window.location.hash||"";if(u.forEach(c=>c.classList.remove("active")),v==="#top"||s==="/"||s===""){n.querySelector('li[data-route="home"]')?.classList.add("active");return}u.forEach(c=>{const y=c.querySelector("a");if(!y)return;const L=(y.getAttribute("href")||"").split("/").pop()||"";if(L===i){c.classList.add("active");return}if((i===""||i==="/")&&L==="index.html"){c.classList.add("active");return}})};a(),window.addEventListener("popstate",a),u.forEach(s=>{const i=s.querySelector("a");i&&s.addEventListener("click",v=>{const c=v.target;c&&c.tagName.toLowerCase()==="a"||(v.preventDefault(),window.location.assign(i.href))})});const t=n.querySelector('li[data-route="home"] a');t&&t.addEventListener("click",()=>{n.classList.contains("open")&&(n.classList.remove("open"),o.setAttribute("aria-expanded","false"))});const r=e.querySelector(".navbar"),h=window.matchMedia("(max-width: 768px)"),p=()=>{r&&(window.scrollY>20?(r.classList.add("navbar-solid"),r.classList.remove("navbar-transparent")):(r.classList.remove("navbar-solid"),r.classList.add("navbar-transparent")))};p(),window.addEventListener("scroll",p,{passive:!0}),window.addEventListener("resize",p),o.addEventListener("click",s=>{s.stopPropagation();const i=n.classList.toggle("open");o.setAttribute("aria-expanded",String(i)),h.matches&&(i?(r?.classList.add("navbar-solid"),r?.classList.remove("navbar-transparent")):p())}),document.addEventListener("click",s=>{!e.contains(s.target)&&n.classList.contains("open")&&(n.classList.remove("open"),o.setAttribute("aria-expanded","false"),h.matches&&p())}),document.addEventListener("keydown",s=>{s.key==="Escape"&&n.classList.contains("open")&&(n.classList.remove("open"),o.setAttribute("aria-expanded","false"),h.matches&&p())})}}function q(e){try{document.body.classList.add("page-home")}catch{}e.innerHTML=`
  ...
  `}const O="/assets/trailer-poster-Bm6cdYvO.jpg";function T(e){e.innerHTML=`
    <footer class="footer">
      <div class="container">
